...
export default class CustomServer extends net.Server {
  /**
   * @typedef {import("../protocol/protocol-bridge.js").default} ProtocolBridge
   * @param {ProtocolBridge} ProtocolBridge 
   */
  constructor(ProtocolBridge) {
    const protocolBridge = ProtocolBridge;
    super((socket) => {
      if (
        metrics.getSnapshot().activeConnections >=
        EnvConfig.get_max_connections()
      ) {
        log(
          "WARN",
      ...
      socket.on("data", async (chunk) => {
        try {
          ...
          if (Buffer.isBuffer(response)) {
            ...
            socket.write(
              protocolBridge.handleSocketError("No response generated"),
            );
          }
          socket.resume();
        } catch (error) {
          ...
          metrics.incrementErrorCount();
          socket.write(
            protocolBridge.handleSocketError("Data handling failed"),
          );
          socket.destroy();
          socket.removeAllListeners();
        }
