 ...
 */
export async function handle(op, ns, key, data) {
  try {
...
async function create(ns, data) {
  try {
    ...
    if (!Array.isArray(records)) {
      log("ERROR", `[Database.create] Expected array`, {
        ns,
        type: typeof records,
      });
      return [null, StatusCodes.INTERNAL_SERVER_ERROR];
 ...
 * @example
 * // Get all users
 * const [users, status] = await read('users', null);
 *
 * // Get specific user
 * const [user, status] = await read('users', 'user123');
 */
async function read(ns, key) {
  try {
    if (typeof ns !== "string" || !ns) {
 ...
 */
async function update(ns, key, data) {
  try {
    if (typeof ns !== "string" || !ns) {
    ...
    if (!Array.isArray(existingData)) {
      log("ERROR", `[Database.update] Expected array`, {
        ns,
 ...
 *
 * @example
 * const [deleted, status] = await remove('users', 'user123');
...
async function remove(ns, key) {
  try {
    ...
    if (!Array.isArray(existingData)) {
      log("ERROR", `[Database.delete] Expected array`, {
        ns,
        type: typeof existingData,
      });
      return [null, StatusCodes.INTERNAL_SERVER_ERROR];
